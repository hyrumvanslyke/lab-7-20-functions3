...
// CODE HERE
 numbers.forEach((x) => console.log(x/2))// for each item in the array divide tht value by 2
////////// PROBLEM 2 //////////
...
// CODE HERE
let divideReturn = numbers.map((x) => x/= 2)// map a new array that has the previous arrays values divided by 2
console.log(divideReturn)// logged to test
...
// CODE HERE
let iCities = cities.filter((x) => x.endsWith('i'))// filter cities that end with i
console.log(iCities)// logged to test
////////// PROBLEM 4 //////////
...
fruits.forEach(function(elem, i){// fruits for each method checking for fruit color
  if(elem.color === 'red'){// if red its an apple
    console.log(`The fruit with index ${i} is an apple`)
  }else if(elem.color === 'orange'){// if orange it is an orange
...
foods.forEach((food) => {// writing a for each loop for the foods array with food as a parameter
  if (food.tags.includes("rice")) {// if statement that if in the food tags it includes rice add it to the total price 
    totalPrice += food.price
  }
})

// THE TOTAL
console.log(`The total price for foods with rice in them is $${totalPrice}`)// logged for testing
